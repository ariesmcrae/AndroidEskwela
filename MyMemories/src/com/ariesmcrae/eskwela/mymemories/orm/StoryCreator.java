...
package com.ariesmcrae.eskwela.mymemories.orm;

import java.util.ArrayList;

import com.ariesmcrae.eskwela.mymemories.provider.EskwelaSchema;
...
public class StoryCreator {
	...
	public static ContentValues getCVfromStory(final StoryData data) {
		...
		rValue.put(EskwelaSchema.Story.Cols.LOGIN_ID, data.loginId);
		rValue.put(EskwelaSchema.Story.Cols.STORY_ID, data.storyId);
		rValue.put(EskwelaSchema.Story.Cols.TITLE, data.title);
		rValue.put(EskwelaSchema.Story.Cols.BODY, data.body);
		rValue.put(EskwelaSchema.Story.Cols.AUDIO_LINK, data.audioLink);
		rValue.put(EskwelaSchema.Story.Cols.VIDEO_LINK, data.videoLink);
		rValue.put(EskwelaSchema.Story.Cols.IMAGE_NAME, data.imageName);
		rValue.put(EskwelaSchema.Story.Cols.IMAGE_LINK, data.imageLink);
		rValue.put(EskwelaSchema.Story.Cols.TAGS, data.tags);
		rValue.put(EskwelaSchema.Story.Cols.CREATION_TIME, data.creationTime);
		rValue.put(EskwelaSchema.Story.Cols.STORY_TIME, data.storyTime);
		rValue.put(EskwelaSchema.Story.Cols.LATITUDE, data.latitude);
		rValue.put(EskwelaSchema.Story.Cols.LONGITUDE, data.longitude);
		return rValue;
	}
	...
	public static StoryData getStoryDataFromCursor(Cursor cursor) {
		...
		long rowID = cursor.getLong(cursor.getColumnIndex(EskwelaSchema.Story.Cols.ID));
		long loginId = cursor.getLong(cursor.getColumnIndex(EskwelaSchema.Story.Cols.LOGIN_ID));
		long storyId = cursor.getLong(cursor.getColumnIndex(EskwelaSchema.Story.Cols.STORY_ID));
		String title = cursor.getString(cursor.getColumnIndex(EskwelaSchema.Story.Cols.TITLE));
		String body = cursor.getString(cursor.getColumnIndex(EskwelaSchema.Story.Cols.BODY));
		String audioLink = cursor.getString(cursor.getColumnIndex(EskwelaSchema.Story.Cols.AUDIO_LINK));
		String videoLink = cursor.getString(cursor.getColumnIndex(EskwelaSchema.Story.Cols.VIDEO_LINK));
		String imageName = cursor.getString(cursor.getColumnIndex(EskwelaSchema.Story.Cols.IMAGE_NAME));
		String imageMetaData = cursor.getString(cursor.getColumnIndex(EskwelaSchema.Story.Cols.IMAGE_LINK));
		String tags = cursor.getString(cursor.getColumnIndex(EskwelaSchema.Story.Cols.TAGS));
		long creationTime = cursor.getLong(cursor.getColumnIndex(EskwelaSchema.Story.Cols.CREATION_TIME));
		long storyTime = cursor.getLong(cursor.getColumnIndex(EskwelaSchema.Story.Cols.STORY_TIME));
		double latitude = cursor.getDouble(cursor.getColumnIndex(EskwelaSchema.Story.Cols.LATITUDE));
		double longitude = cursor.getDouble(cursor.getColumnIndex(EskwelaSchema.Story.Cols.LONGITUDE));
