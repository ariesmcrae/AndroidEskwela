 ...
 * 
 */
public class EskwelaResolver {

	private ContentResolver cr;

	private Uri storyURI = EskwelaSchema.Story.CONTENT_URI;
	private Uri tagsURI = EskwelaSchema.Tags.CONTENT_URI;
	 ...
	 */
	public EskwelaResolver(Activity activity) {
	...
	public ContentProviderResult[] applyBatch(final ArrayList<ContentProviderOperation> operations) throws RemoteException,
		...
		return cr.applyBatch(EskwelaSchema.AUTHORITY, operations);
	}
	...
	public Uri insert(final StoryData storyObject) throws RemoteException {
		...
		tempCV.remove(EskwelaSchema.Story.Cols.ID);
	...
	public Uri insert(final TagsData tagsObject) throws RemoteException {
		...
		tempCV.remove(EskwelaSchema.Tags.Cols.ID);
	...
	public StoryData getStoryDataViaRowID(final long rowID) throws RemoteException {
		...
		ArrayList<StoryData> results = queryStoryData(null, EskwelaSchema.Story.Cols.ID + "= ?", selectionArgs, null);
		if (results.size() > 0) {
	...
	public TagsData getTagsDataViaRowID(final long rowID) throws RemoteException {
		String[] selectionArgs = { String.valueOf(rowID) };
		ArrayList<TagsData> results = queryTagsData(null, EskwelaSchema.Tags.Cols.ID + "= ?", selectionArgs, null);
		if (results.size() > 0) {
			return results.get(0);
		} else {
			return null;
	...
	public int deleteAllStoryWithRowID(long rowID) throws RemoteException {
		...
		return deleteStoryData(EskwelaSchema.Story.Cols.ID + " = ? ", args);
	}
	...
	public int deleteAllTagsWithRowID(long rowID) throws RemoteException {
		String[] args = { String.valueOf(rowID) };
		return deleteTagsData(EskwelaSchema.Tags.Cols.ID + " = ? ", args);
	}
